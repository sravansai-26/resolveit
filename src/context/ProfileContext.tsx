...
type ProfileContextType = {
  ...
  toggleRepost: (issueId: string) => Promise<void>; // Added for un-reposting
};
...
export function ProfileProvider({ children }: { children: ReactNode }) {
  ...
  const fetchReposts = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/issues/reposts/me', {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const toggleRepost = async (issueId: string) => {
    try {
      ...
      if (res.ok && json.success) {
        ...
        // You might also want to update the original issues list (if displayed elsewhere)
        // to reflect the correct repostedByUser status, but for "My Reposts", removal is the key.
      } else {
